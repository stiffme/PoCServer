 ...
 */
case class SigRequestData(impu:String)
case class SigRequestDataResult(repoData:Option[RepoData])
case class SigUpdateData(impu:String,data:Map[String,Int])
case class SigUpdateDataResult(success:Boolean)
case class SigDeleteData(impu:String)
case class SigDeleteDataResult(success:Boolean)
case class RepoData(seq:Int,data:Map[String,Int])

class DiameterActor extends Actor with ActorLogging{
  ...
  var diameterServer:DiameterServer = null

  @throws[Exception](classOf[Exception])
  override def preStart(): Unit = {
    super.preStart()
    //start diameter server
    diameterServer = DiameterServer.createServer(destinationHost,destinationPort,destinationAddress,originHost,originRealm)
  ...
  def receive = {
    case SigRequestData(impu) => {
      sender ! SigRequestDataResult(requestData(impu))
    }
    case SigUpdateData(impu,data) => {
      sender ! SigUpdateDataResult(updateData(impu,data))
    }
    case SigDeleteData(impu) => {
      sender ! SigDeleteDataResult(deleteUserData(impu))
    }
  }


  private def requestData(impu:String):Option[RepoData] = {
    val udr = makeCommonMessage(ShUDR,impu)
    udr add new AVP_UTF8String(ServiceIndication,TGPP,serviceIndication).setM
    val answer = diameterServer.sendRequest(udr)
  ...
  private def updateData(impu:String,data:Map[String,Int]):Boolean = {
    //get current data first
    requestData(impu) match {
      case None => {
      ...
      case Some(repoData) => {
        ...
        val answer = diameterServer.sendRequest(pur)
        isSuccessfulAnswer(answer)
      }
    }
  }

  private def deleteUserData(impu:String):Boolean = {
    requestData(impu) match {
      case None => {
